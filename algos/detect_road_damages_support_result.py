...
def check_for_result_input(input_dir_path, output_dir_path):
    # Perform a recursive search for "detections.json" files in the input directory
    detections_paths = glob.glob(os.path.join(input_dir_path, '**', 'detections.json'), recursive=True)
    
    if detections_paths:
        print('detections.json file found, reusing input')
        # Use the first detections.json file found
        detections_path = detections_paths[0]
        # Determine the directory of the current detections.json file
        detections_dir_path = os.path.dirname(detections_path)
        metadata_path = os.path.join(detections_dir_path, 'metadata.json')
        images_dir_path = os.path.join(detections_dir_path, 'images')
        
        if os.path.exists(metadata_path) and os.path.isdir(images_dir_path):
            # Define the new "result" directory path within the output directory
            result_dir_path = os.path.join(output_dir_path, 'result')
            
            # Ensure the target directory does not exist to avoid shutil.copytree error
            if os.path.exists(result_dir_path):
                shutil.rmtree(result_dir_path)
            
            # Copy the directory containing detections.json to "result"
            shutil.copytree(detections_dir_path, result_dir_path)
            
            # Delete the file named "0" in the copied directory, if it exists
            file_to_delete = os.path.join(result_dir_path, '0')
            if os.path.exists(file_to_delete):
                os.remove(file_to_delete)
            
            # Create a ZIP archive of the "result" directory
            archive_path = shutil.make_archive(result_dir_path, 'zip', result_dir_path)
            print(f"Archive created at: {archive_path}")
            
            # remove the "result" directory after archiving
            shutil.rmtree(result_dir_path)
            
            print('=== finished ===')
            sys.exit() # STOP SCRIPT EXECUTION TO PREVENT YOLO
        else:
            print("The required 'metadata.json' or 'images' directory does not exist in the same directory as the first 'detections.json' found.")
        ...
        try:
            ...
            with Image.open(path_str) as img:
                ...
                if img.format == 'JPEG':
                    ...
                    valid_images[filename] = image_info
...
def load_images(input_dir_path):
    jpg_file_ending_paths = []  # List to hold paths of jpg images
    for root, dirs, files in os.walk(input_dir_path):
        for file in files:
            if file.lower().endswith('.jpg') or file.lower().endswith('.jpeg'): # TODO replace with mimetypes.guess_type(url, strict=True)
...
def create_metadata_dict(detections):
    ...
    for damage_list in detections:
        for damage in damage_list['roadDamages']:
            damage_count += 1 # TODO use len , move up one for loop
...
def main():
    ...
    output_dir_path = './data/outputs' if is_local_test else '/data/outputs'
    ...
    if not is_local_test:
        ...
        DID = DIDS[0]
        input_dir_path = f'/data/inputs/{DID}'
